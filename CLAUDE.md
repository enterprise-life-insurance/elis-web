...
**Social link previews (OG/Twitter tags, added 2026-08-10):** `Layout.astro` renders `og:*`/`twitter:*` meta tags plus a canonical link from each page's `title`/`description` props (already passed everywhere) and an optional `image` prop (advisor pages use their headshot; everything else falls back to the generated `public/og-image.png`, 1200×630). These need an absolute `site` URL in `astro.config.mjs` to resolve — it's currently set to `https://elis-web.vercel.app` (**not** `enterpriselife.ca`), because the real domain is still parked at the registrar (redirects to `/lander`, doesn't serve this deployment) and would otherwise 404 for link-preview crawlers. ⚠️ Swap `site` back to `https://enterpriselife.ca` once DNS is actually pointed at this Vercel project — don't do it before confirming the domain resolves to the live build, or previews break again.
...
- **CMS:** Sanity, embedded at `/studio` in this repo via `@sanity/astro` (confirmed 2026-08-07 — not standalone). Config in `sanity.config.ts`; schema types in `src/sanity/schemaTypes/` (`advisor`, `service`, `segment`, `resource`, `testimonial`, singleton `siteSettings`, plus a reusable `faqItem` object). No real Sanity project exists yet — `astro.config.mjs` falls back to a placeholder `projectId` so the site still builds; see `.env.example` for the real `PUBLIC_SANITY_*` vars to set once a project is created. `scripts/seed-sanity.ts` (`pnpm seed`) pushes the existing `src/data/*.ts` content into Sanity as real documents via `createOrReplace` once a project exists — needs `SANITY_API_WRITE_TOKEN` in `.env`; rerunning it after editing `src/data/*.ts` re-overwrites those documents, so it's a one-time bridge, not a sync loop — any content since edited directly in Studio for the same documents would get clobbered.
...
- `brand/brand-voice-messaging.md` — locked positioning, mission, vision, communication pillars (with "say this / not that" examples), and segment-specific tone (Engineers, Healthcare, Business Owners & Contractors, Families & Individuals). Any UI copy should follow these pillars and tone rules.
