...
- Animation budget/library: safepacific.com's consistent 300ms hover system and the two Framer sites' scroll-appear + glassmorphism are the most reusable *patterns*, but none of them are trivial to replicate outside Framer without picking a real animation stack (e.g. GSAP/Framer Motion + Lenis for smooth scroll) — that's a tech-stack decision, not just a style one.
- Dark-mode/neon-accent formula (subduxion, alethia): striking and clearly what's driving the "premium and unique" read, but it's a fairly different visual register from ELIS's current warm/family-photography direction (visual identity draft + local wireframe) and from limeiq's warm-neutral insurance-tech palette. Worth clarifying with Jay whether "premium" is meant as *mood* (confidence, restraint, quality of motion) to reinterpret in ELIS's own light palette, or as a literal dark-UI direction.
